...
WS_ALLOW_PUB=""
WS_ALLOW_SUB=""
WS_ALLOW_CONSUMER_CREATE=""
WS_DENY_CONSUMER_CREATE=""

IFS=',' read -ra WS_PUB_STREAMS <<< "$WEBSOCKET_PUB_STREAMS"
for STREAM in "${WS_PUB_STREAMS[@]}"; do
  STREAM=$(echo "$STREAM" | xargs)
  WS_ALLOW_PUB="${WS_ALLOW_PUB:+${WS_ALLOW_PUB},}${STREAM}.>"
done

IFS=',' read -ra WS_SUB_STREAMS <<< "$WEBSOCKET_SUB_STREAMS"
for STREAM in "${WS_SUB_STREAMS[@]}"; do
  STREAM=$(echo "$STREAM" | xargs)
  WS_ALLOW_SUB="${WS_ALLOW_SUB:+${WS_ALLOW_SUB},}${STREAM}.>"
done

IFS=',' read -ra WS_ALLOW_CONSUMER_STREAMS <<< "$WEBSOCKET_ALLOW_CONSUMER_CREATE_STREAMS"
for STREAM in "${WS_ALLOW_CONSUMER_STREAMS[@]}"; do
  STREAM=$(echo "$STREAM" | xargs)
  WS_ALLOW_CONSUMER_CREATE="${WS_ALLOW_CONSUMER_CREATE}\$JS.API.CONSUMER.CREATE.${STREAM}.>,"
done

IFS=',' read -ra WS_DENY_CONSUMER_STREAMS <<< "$WEBSOCKET_DENY_CONSUMER_CREATE_STREAMS"
for STREAM in "${WS_DENY_CONSUMER_STREAMS[@]}"; do
  STREAM=$(echo "$STREAM" | xargs)
  WS_DENY_CONSUMER_CREATE="${WS_DENY_CONSUMER_CREATE}\$JS.API.CONSUMER.CREATE.${STREAM}.>,"
done

WS_ALLOW_PUB="${WEBSOCKET_ALLOW_PUB:+${WEBSOCKET_ALLOW_PUB},}${WS_ALLOW_PUB}"
WS_ALLOW_SUB="${WEBSOCKET_ALLOW_SUB:+${WEBSOCKET_ALLOW_SUB},}${WS_ALLOW_SUB}"
WS_ALLOW_CONSUMER_CREATE=${WS_ALLOW_CONSUMER_CREATE%,}
WS_DENY_CONSUMER_CREATE=${WS_DENY_CONSUMER_CREATE%,}
